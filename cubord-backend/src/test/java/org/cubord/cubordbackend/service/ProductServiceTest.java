...
class ProductServiceTest {
    ...
    class DeprecatedMethodsTests {
        ...
        @Test
        @DisplayName("deprecated createProduct should delegate to new method")
        void deprecatedCreateProductShouldDelegate() {
            // Given
            mockAuthenticatedUser(testUserId);
            when(productRepository.findByUpc(any())).thenReturn(Optional.empty());
            when(upcApiService.fetchProductData(any())).thenThrow(new RuntimeException("API failed"));
            when(productRepository.save(any(Product.class))).thenAnswer(inv -> {
                Product p = inv.getArgument(0);
                p.setId(productId);
                return p;
            ...
            ProductResponse response = productService.createProduct(testProductRequest, null);
        ...
        @Test
        @DisplayName("deprecated getProductById should delegate to new method")
        void deprecatedGetProductByIdShouldDelegate() {
            // Given
            mockAuthenticatedUser(testUserId);
            when(productRepository.findById(eq(productId))).thenReturn(Optional.of(testProduct));
            ...
            // When
            @SuppressWarnings("deprecation")
            ProductResponse response = productService.getProductById(productId, null);
        ...
        @Test
        @DisplayName("deprecated getProductByUpc should delegate to new method")
        void deprecatedGetProductByUpcShouldDelegate() {
            // Given
            String upc = "123456789012";
            mockAuthenticatedUser(testUserId);
            when(productRepository.findByUpc(eq(upc))).thenReturn(Optional.of(testProduct));
            ...
            // When
            @SuppressWarnings("deprecation")
            ProductResponse response = productService.getProductByUpc(upc, null);
