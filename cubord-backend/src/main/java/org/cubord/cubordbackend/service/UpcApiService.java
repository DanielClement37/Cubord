...
public class UpcApiService {
     ...
     * 
     * <p><strong>SSRF Protection:</strong> The constructed URL is validated before making
     * the external request to prevent Server-Side Request Forgery attacks.</p>
     *
    ...
    public ProductResponse fetchProductData(String upc) {
        ...
        try {
            ...
            log.trace("Constructed API URL: {}", url);

            // SSRF Protection: Validate the URL before making the request
            try {
                UrlValidator.isValidExternalUrl(url);
                log.trace("URL validation successful for: {}", url);
            } catch (UrlValidationException e) {
                log.error("SECURITY: SSRF attempt detected - URL validation failed for UPC: {}, URL: {}", 
                        upc, url, e);
                throw new ExternalServiceException("Open Food Facts API", 
                    "URL validation failed: " + e.getMessage(), e);
            }
            ...
            // Re-throw SSRF validation errors
            log.error("SECURITY: URL validation failed when fetching UPC: {}", upc, e);
            throw new ExternalServiceException("Open Food Facts API", 
     ...
     * 
     * <p>The URL is constructed from the configured base API URL and the UPC code.
     * The resulting URL will be validated for SSRF protection before use.</p>
     *
